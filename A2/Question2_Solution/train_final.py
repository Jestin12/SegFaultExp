...
def train(model, train_loader, optimizer, criterion, epoch, epochs):
    ...
    for batch_idx, (inputs, targets) in enumerate(pbar):
        ...
        # Statistics
        # f calculate the loss and acc
        train_loss += loss.item()

        # Calculate predictions
        _, predicted = torch.max(outputs, 1)  # Get the predicted class
        total += targets.size(0)  # Add the batch size to total
    ...
    avg_loss = train_loss / len(train_loader)  # Average loss
    acc = 100. * correct / total  # Accuracy as a percentage    
...
def validate(model, val_loader, criterion):
    ...
    # Calculate average loss and accuracy
    avg_loss = val_loss / len(val_loader)  # Average loss
    acc = 100. * correct / total  # Accuracy as a percentage
