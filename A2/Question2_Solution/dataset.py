...
class TrafficSignProcessor:
    ...
    def __init__(self, config=None):
        ...
        self.config = {
            ...
            'class_mapping': {
                14: 0,          #   Image names for the traffic signs are 14, 33, 34, 35, 40 in the training data
                33: 1,
                34: 2,
                35: 3,
                40: 4
            },
            'img_size': 32,     #   The largest image size in the training data is 32x32 bits
    ...
    def create_datasets(self, augment_train=True, include_original=False):
        ...
        if augment_train:
            ...
            '''
            Applying transforms onto the training data to produce more training data
            with images of various prespectives, lighting, contrast and saturations           
            '''
            ...
            train_transform = transforms.Compose([
                ...
                # Data augmentation
                transforms.RandomRotation(degrees=30),  # Rotate randomly between -30 to +30 degrees
                transforms.ColorJitter(brightness=0.2, contrast=0.6, saturation=0.2),  # Adjust brightness, contrast, and saturation
                transforms.RandomAffine(degrees=25, translate=(0.1, 0.1)),  # Random affine transformations
                transforms.RandomPerspective(distortion_scale=0.5, p=0.75),  # Random perspective distortion
                transforms.RandomPosterize(bits=2),  # Reduce color depth
                transforms.RandomSolarize(threshold=150),  # Simulate overexposure
